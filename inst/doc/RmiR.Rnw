...
%\VignetteDepends{RmiR.db, RmiR, hgug4112a.db}
...
The package uses various databases of microRNA targets from the \Rpackage{RmiR.db} package:
\begin{itemize}
...
\par For more details about using the databases have a look at \Rpackage{RmiR.db} package vignette.
